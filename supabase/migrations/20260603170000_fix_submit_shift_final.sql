-- Definitive fix for submit_shift: drop ALL overloads, create only the
-- p_cashier_id version. The 12:00 cashier_shifts migration recreated the
-- broken auth.uid() variant after the 08:37 fix, so both overloads existed.
