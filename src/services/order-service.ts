...
export async function createOrder(
  ...
  try {
    ...
    //    via the admin client without depending on anon SELECT RLS.
    ...
    // Fetch the inserted order back (anon SELECT policy allows reading orders < 2 min old)
    ...
    if (fetchError || !order) {
      console.error('createOrder: Failed to fetch inserted order:', fetchError);
      return serviceError('E2002', 'Failed to create order. Please try again.');
    }
